...
const THEMES={
  ...
  dark:{name:"שחור",bg:"linear-gradient(160deg,#0a0a0a,#1a1a2e,#16213e)",accent:"#5DA8E8"},
  green:{name:"ירוק",bg:"linear-gradient(160deg,#0a1f0a,#1a3a1a,#0d2b0d)",accent:"#4CAF7D"},
  navy:{name:"כחול",bg:"linear-gradient(160deg,#020b18,#0a2540,#0d3b6e)",accent:"#5DA8E8"},
  midnight:{name:"לילה",bg:"linear-gradient(160deg,#1a0533,#2d0b5a,#1a0533)",accent:"#c084fc"},
  warm:{name:"חם",bg:"linear-gradient(160deg,#1a0a00,#3d1a00,#2a1200)",accent:"#E8A838"},
};
...
let S={
  ...
  fullNote:null,fullNoteEditing:false,reorderMode:false,editingTitle:false,
...
function createBackup(){
  ...
  a.href=url;
  const d=new Date();
  a.download=`גיבוי-${d.getDate()}-${d.getMonth()+1}-${d.getFullYear()}.json`;
...
function restoreFromBackup(json){
  try{
    ...
    S.cats=b.cats||DEFAULT_CATS;S.catsTrash=b.catsTrash||[];
    S.data=b.data||(()=>{const d={};DEFAULT_CATS.forEach(c=>{d[c.id]={goals:[]}});return d;})();
    S.goalsTrash=b.goalsTrash||[];S.torahBooks=b.torahBooks||[];S.torahTrash=b.torahTrash||[];
    S.npFolders=b.npFolders||[{id:"default",name:"כללי"}];S.npNotes=b.npNotes||[];
    S.npTrash=b.npTrash||[];S.bookImages=b.bookImages||{};
    ...
    setState({view:"home",showSettings:false});
...
function goBack(){
  resetForms();
  if(S.fullNote){setState({fullNote:null,fullNoteEditing:false});return;}
...
function deleteBookImage(id){delete S.bookImages[id];setState({});}
...
function buildHeader(){
  ...
  const showBack=S.view!=="home"||S.fullNote||S.showSettings;
  let back=showBack?`<button class="hb" onclick="goBack()" style="font-size:16px;padding:4px 11px">←</button>`:"";
  ...
  if(S.view==="home"&&S.editingTitle){
    ...
    titleEl=`<div class="hdr-title" onclick="${clickFn}">${title}${clickable?" <span style='font-size:11px;opacity:0.35'>✎</span>":""}</div>`;
  }
  ...
  if(!S.showSettings&&!S.fullNote){
    ...
    if(S.view==="goal-detail"&&curGoal){
      ...
      actions+=`<button class="hb" onclick="shareEmail(decodeURIComponent('${encodeURIComponent(curGoal.title)}'),decodeURIComponent('${wtxt}'))" style="color:#EA4335;background:rgba(234,67,53,0.1)">✉️</button>`;
    }
  ...
  if(S.fullNote){
    ...
    const titleLabel=isNp?"פתק":"הערה";
    actions+=`<button class="hb" onclick="shareWA(decodeURIComponent('${wtxt}'))" style="color:#25D366;background:rgba(37,211,102,0.15)">📱</button>`;
    actions+=`<button class="hb" onclick="shareEmail('${titleLabel}',decodeURIComponent('${wtxt}'))" style="color:#EA4335;background:rgba(234,67,53,0.12)">✉️</button>`;
    if(isNp)actions+=hb(S.fullNoteEditing?"✓ שמור":"✏️ ערוך",S.fullNoteEditing?"saveFullNoteEdit()":"setState({fullNoteEditing:true})","background:rgba(255,255,255,0.12)");
  }
...
function buildFooter(){
  const allItems=[["home","🏠"],["notepad","📓"],["torah-progress","📖"],...S.cats.map(c=>[c.id+"_g",c.emoji])];
  const visible=allItems.filter(([v])=>{
    if(v==="home")return true;
    if(v==="notepad")return S.visibleSections.includes("notepad");
    if(v==="torah-progress")return S.visibleSections.includes("torah");
    return S.visibleSections.includes("goals-grid");
  }).slice(0,8);

  return`<div class="footer">${visible.map(([v,e])=>{
...
function saveFullNoteEdit(){
  ...
  if(!v.trim())return;
  if(S.fullNote.type==="np"){
    S.npNotes=S.npNotes.map(n=>n.id===S.fullNote.id?{...n,text:v.trim()}:n);
  }else{
    const g=S.data[S.activeCat]?.goals.find(g=>g.id===S.activeGoalId);
    if(g){const n=g.notes.find(n=>n.id===S.fullNote.id);if(n)n.text=v.trim();}
  }
  setState({fullNoteEditing:false});
}
...
function buildSettings(){
  ...
  [["notepad","📓 NOTEPAD"],["torah","📖 לימוד תורה"],["goals-grid","🎯 מטרות"]].forEach(([key,label])=>{
    const on=S.visibleSections.includes(key);
    h+=`<div class="toggle-row">
      <span style="font-size:14px">${label}</span>
      <button class="toggle" onclick="toggleSection('${key}')" style="background:${on?accent:"rgba(255,255,255,0.15)"}"><span style="position:absolute;width:20px;height:20px;border-radius:50%;background:#fff;top:2px;left:${on?"22px":"2px"};transition:left 0.2s;display:block"></span></button>
    </div>`;
  });
  h+=`</div>`;
  ...
  h+=`<div class="settings-section">
    ...
    <div class="toggle-row">
      <div><div style="font-size:14px;font-weight:600">מצב קומפקטי</div><div style="font-size:11px;color:rgba(255,255,255,0.45)">כרטיסים צפופים יותר</div></div>
  ...
  h+=`<div class="settings-section">
    ...
    <div style="display:flex;flex-direction:column;gap:8px">
      <button onclick="createBackup()" style="width:100%;background:rgba(76,175,125,0.2);border:1px solid rgba(76,175,125,0.4);border-radius:12px;color:#4CAF7D;padding:13px;font-size:14px;font-weight:600;cursor:pointer;text-align:right">💾 צור קובץ גיבוי<div style="font-size:11px;font-weight:400;color:rgba(255,255,255,0.5);margin-top:3px">מוריד את כל הנתונים לקובץ</div></button>
      <button onclick="document.getElementById('backupInput').click()" style="width:100%;background:rgba(93,168,232,0.2);border:1px solid rgba(93,168,232,0.4);border-radius:12px;color:#5DA8E8;padding:13px;font-size:14px;font-weight:600;cursor:pointer;text-align:right">📂 שחזר מקובץ גיבוי<div style="font-size:11px;font-weight:400;color:rgba(255,255,255,0.5);margin-top:3px">טען קובץ JSON לשחזור הכל</div></button>
    </div>
  </div>`;
...
function toggleSection(key){
  const v=[...S.visibleSections];
  const i=v.indexOf(key);
  if(i>=0){if(v.length>1)v.splice(i,1);}else{v.push(key);}
  setState({visibleSections:v});
}
...
function buildHome(){
  ...
  S.homeOrder.forEach((item,idx)=>{
    if(!S.visibleSections.includes(item))return;
    if(item==="notepad"){
  ...
  return trashSection(S.catsTrash,"setState({catsTrash:[]})",cat=>`
    <div class="card" style="border:1px solid ${cat.color}44;background:${cat.color}0a">
      <div class="row" style="margin-bottom:6px">
        <div style="display:flex;align-items:center;gap:8px"><span style="font-size:20px">${cat.emoji}</span><div><div style="font-weight:700;font-size:14px">${cat.label}</div><div style="font-size:11px;color:rgba(255,255,255,0.4)">נמחק: ${cat.deletedAt}</div></div></div>
...
function buildGoals(){
  ...
  if(S.addingGoal){
    h+=`<div class="card" style="margin-bottom:10px">${inp("newGoalInp","שם המטרה...","","text","style='margin-bottom:10px'")}<div style="display:flex;gap:8px">${btn("הוסף ✓","addGoalFromForm()",cat.color,"flex")}${btn("ביטול","setState({addingGoal:false})")}</div></div>`;
  }
  goals.forEach((g,i)=>{
  ...
  goal.notes.forEach((n,i)=>{
    ...
    h+=`<div class="card" style="border:1px solid ${cat.color}33;opacity:${n.done?0.5:1}">
      <div style="display:flex;align-items:flex-start;gap:9px">
        ...
        <div style="flex:1;cursor:pointer" onclick="cycleNoteExp('${n.id}')">
          ${tag("#"+(i+1),cat.color)}
    ...
    h+=`<div class="card" style="border:1px solid rgba(76,175,125,0.3)">
      <div class="row" style="margin-bottom:8px">
        ...
        <div style="display:flex;gap:5px">
          ...
          ${hasImg?ib("🗑🖼",`deleteBookImage('${b.id}')`, "rgba(255,80,80,0.15)"):""}
...
function pickPhoto(id){S.pendingPhotoBookId=id;document.getElementById("galleryInput").click();}
...
function buildFolderMenu(){
  ...
  h+=`<div style="border-top:1px solid rgba(255,255,255,0.08);margin-top:9px;padding-top:8px;display:flex;flex-direction:column;gap:5px">
    <button class="hb" onclick="importTxt()" style="font-size:12px;text-align:center;color:#5DA8E8;background:rgba(93,168,232,0.1)">⬆ ייבוא TXT</button>
...
function buildNotepad(){
  ...
  if(S.addingNpNote){
    h+=`<div class="card" style="margin-bottom:10px"><div style="font-size:11px;color:rgba(255,255,255,0.35);margin-bottom:5px">💡 ==טקסט== = הדגשה צהובה</div>${textarea("newNotepadTA","כתוב פתק...",S.npNoteText,5)}<div style="display:flex;gap:8px;margin-top:8px">${btn("שמור ✓","saveNpNoteFromForm()","#5DA8E8","flex")}${btn("ביטול","setState({addingNpNote:false,npNoteText:''})")}</div></div>`;
  }
  ...
  notes.forEach(n=>{
    ...
    if(S.editNpNoteId===n.id){
      h+=`<div class="card"><div style="font-size:11px;color:rgba(255,255,255,0.35);margin-bottom:5px">💡 ==טקסט== = הדגשה צהובה</div>${textarea("editNoteTA","",n.text,4)}<div style="display:flex;gap:7px;margin-top:8px">${btn("שמור ✓","saveEditNoteFromForm()","#5DA8E8","flex")}${btn("ביטול","setState({editNpNoteId:null,npNoteText:''})")}</div></div>`;
    }else{
      h+=`<div class="card" style="border:1px solid ${n.star?"rgba(255,210,0,0.4)":"rgba(255,255,255,0.1)"};background:${n.star?"rgba(255,210,0,0.05)":"rgba(255,255,255,0.07)"}">
...
function buildFullNote(){
  ...
  if(S.fullNoteEditing){
    body=`<div style="padding:20px;flex:1;display:flex;flex-direction:column">
      ${date?`<div style="font-size:11px;color:rgba(255,255,255,0.4);margin-bottom:10px">📅 ${date}</div>`:""}
      <textarea id="fullNoteEditTA" class="inp" style="flex:1;min-height:300px;font-size:15px;line-height:1.7">${text.replace(/</g,"&lt;").replace(/>/g,"&gt;")}</textarea>
      <div style="display:flex;gap:8px;margin-top:12px">${btn("שמור ✓","saveFullNoteEdit()","#5DA8E8","flex")}${btn("ביטול","setState({fullNoteEditing:false})")}</div>
    </div>`;
  }else{
...
function render(){
  ...
  if(S.fullNote){
    shell.innerHTML=buildHeader()+`<div class="content">${buildFullNote()}</div>`+buildFooter();
    if(S.fullNoteEditing){const ta=document.getElementById("fullNoteEditTA");if(ta){ta.focus();ta.setSelectionRange(ta.value.length,ta.value.length);}}
    return;
  }
...
// ══ FILE INPUTS ══
function handleImageFile(file,bookId){
...
document.getElementById("txtInput").addEventListener("change",e=>{
  ...
  r.onload=ev=>{
    ...
    alert("יובאו "+parts.length+" פתקים");
  };
